...
function MintButton({ setState, state, setScript, setPolicyId, tokenName, setTransactionHash }) {
    ...
    async function mintAiken() {
        ...
        const cbor = applyParamsToScript(plutusScript.validators.filter((val: any) => val.title == "lesson02/nft.nft")[0].compiledCode, [outRef, tokenName])
        const script = {
            code: cbor,
            version: "V2"
        }
        ...
        const tx = new Transaction({ initiator: wallet }).mintAsset(script, asset, redeemer).setTxInputs([utxo]);
...
function UnlockButton({ setState, state, script, policyId, tokenName, setTransactionHash }) {
    ...
    async function burnAiken() {
        ...
        const tx = new Transaction({ initiator: wallet }).burnAsset(script, asset, redeemer);
